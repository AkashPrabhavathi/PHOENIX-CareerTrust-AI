...
function App() {
  const [page, setPage] = useState('login') // 'login' | 'form' | 'result' | 'history' | 'about'
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.pageBg, fontFamily: 'sans-serif', transition: 'background 0.3s ease' }}>
      ...
      <div className="no-print" style={{ background: 'linear-gradient(90deg, #028090, #02c39a)', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => setDarkMode(!darkMode)} style={{ background: 'rgba(255,255,255,0.2)', color: '#fff', border: '1px solid #fff', padding: '7px 12px', borderRadius: '20px', cursor: 'pointer', fontSize: '12px' }}>
            {darkMode ? '☀️' : '🌙'}
          </button>
          {isLoggedIn && (
            <>
              <span style={{ fontSize: '13px' }}>👋 {loginData.name}</span>
              <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.2)', color: '#fff', border: '1px solid #fff', padding: '7px 12px', borderRadius: '20px', cursor: 'pointer', fontSize: '12px' }}>
                Logout
              </button>
            </>
          )}
      ...
      {page === 'login' && (
        <>
          <div className="no-print" style={{ textAlign: 'center', padding: '48px 20px 24px' }}>
            ...
            <button onClick={() => document.getElementById('login-card').scrollIntoView({ behavior: 'smooth' })} style={{ marginTop: '28px', background: 'linear-gradient(90deg, #028090, #02c39a)', color: '#fff', border: 'none', padding: '14px 32px', borderRadius: '30px', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer' }}>
          ...
          <div id="login-card" style={{ maxWidth: '400px', margin: '20px auto 40px', background: theme.cardBg, borderRadius: '16px', padding: '26px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
            ...
            />
            <button onClick={handleLogin} style={{ width: '100%', background: '#02c39a', color: '#fff', border: 'none', padding: '13px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>
              Login &amp; Continue →
            </button>
          ...
          <div className="no-print" style={{ maxWidth: '700px', margin: '0 auto 40px', padding: '0 16px' }}>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {[
                ['1️⃣', 'Login', 'Quick free sign-in to get started'],
                ['2️⃣', 'Add Details', 'Paste the offer, upload a screenshot, add your skills'],
          ...
          <div className="no-print" style={{ maxWidth: '700px', margin: '0 auto', padding: '0 16px 40px' }}>
            ...
            <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {[
                ['"Caught a fake internship offer asking for a registration fee before I even applied."', '— B.Tech, Final Year'],
                ['"The skill match feature helped me see what to learn before applying to my dream company."', '— CSE Student'],
                ['"Simple, fast, and honest — it never claims to be 100% sure, which makes me trust it more."', '— MCA Student'],
              ].map((t, i) => (
      ...
      {/* ============ PAGE: FORM ============ */}
      {page === 'form' && (
        <div style={{ maxWidth: '650px', margin: '0 auto', padding: '40px 16px' }}>
          ...
          <div style={{ background: theme.cardBg, borderRadius: '14px', padding: '22px', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
            ...
            <input
              type="text"
              style={{ width: '100%', marginBottom: '20px', padding: '10px', borderRadius: '8px', border: '1px solid #ccc', marginTop: '6px' }}
              value={skills}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setPage('login')} style={{ flex: '0 0 auto', background: 'transparent', color: theme.text, border: `1px solid ${theme.border}`, padding: '14px 18px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
                ← Back
              </button>
              <button
                onClick={handleAnalyze}
                disabled={loading || !jobText}
                style={{ flex: 1, background: 'linear-gradient(90deg, #028090, #02c39a)', color: '#fff', border: 'none', padding: '14px', borderRadius: '10px', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', opacity: loading || !jobText ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              >
      ...
      {/* ============ PAGE: RESULT ============ */}
      {page === 'result' && result && (
        <div style={{ maxWidth: '700px', margin: '0 auto', padding: '30px 16px 50px' }}>

          <div className="no-print" style={{ display: 'flex', gap: '8px', marginBottom: '18px', flexWrap: 'wrap' }}>
            <button onClick={startNewAnalysis} style={{ background: theme.cardBg, color: theme.text, border: `1px solid ${theme.border}`, padding: '9px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
              🔄 New Analysis
            </button>
            <button onClick={() => window.print()} style={{ background: '#0b2e33', color: '#fff', border: 'none', padding: '9px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
              ⬇️ Download / Print
            </button>
            <button onClick={addToComparison} style={{ background: '#2e7d32', color: '#fff', border: 'none', padding: '9px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
              ➕ Add to Comparison
            </button>
            {!result.error && (
              <>
                <button onClick={shareWhatsApp} style={{ background: '#25D366', color: '#fff', border: 'none', padding: '9px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                  📲 Share on WhatsApp
                </button>
                <button onClick={shareEmail} style={{ background: '#1565c0', color: '#fff', border: 'none', padding: '9px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                  ✉️ Share via Email
                </button>
              </>
            )}
            {comparisonList.length > 0 && (
              <button onClick={() => document.getElementById('comparison-section').scrollIntoView({ behavior: 'smooth' })} style={{ background: '#7b1fa2', color: '#fff', border: 'none', padding: '9px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
        ...
        <div style={{ maxWidth: '700px', margin: '0 auto', padding: '40px 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            {historyList.length > 0 && (
              <button onClick={clearHistory} style={{ background: '#c62828', color: '#fff', border: 'none', padding: '8px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '12px' }}>
                Clear All
              </button>
            )}
          </div>
          ...
          {historyList.length === 0 ? (
            <p style={{ color: theme.muted, textAlign: 'center' }}>No past reports yet. Analyze an opportunity to see it here.</p>
          ) : (
                ...
                <div>
                  ...
                  {!entry.result.error && (
                    <div style={{ fontSize: '12px', marginTop: '4px' }}>
                      <span style={{ color: getRiskColor(entry.result.risk_level), fontWeight: 'bold' }}>{entry.result.risk_level}</span>
                      <span style={{ color: theme.muted }}> • Risk {entry.result.risk_score}/100 • Opportunity {entry.result.opportunity_score}/100</span>
                    </div>
                  )}
                </div>
                <button onClick={() => viewHistoryEntry(entry)} style={{ background: 'linear-gradient(90deg, #028090, #02c39a)', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                  View →
                </button>
              </div>
            ))
      ...
      {/* ============ PAGE: ABOUT ============ */}
        ...
        <div style={{ maxWidth: '700px', margin: '0 auto', padding: '40px 16px 60px' }}>
          ...
          <p style={{ color: theme.muted, textAlign: 'center', maxWidth: '520px', margin: '0 auto 30px' }}>
            CareerTrust AI helps students make safer, evidence-based decisions about job and internship offers — combining scam detection, salary analysis, skill matching, and company/recruiter verification in one report.
          </p>
          ...
          <div style={{ background: theme.cardBg, borderRadius: '14px', padding: '24px', marginBottom: '24px', boxShadow: '0 4px 14px rgba(0,0,0,0.06)' }}>
            ...
            <p style={{ color: theme.muted, fontSize: '14px' }}>
              Every year, students lose money and opportunities to fake job offers. This project gives every student — regardless of background — a free, transparent first line of defense before they apply, pay, or share personal information.
            </p>
          </div>
          ...
          <div style={{ background: theme.cardBg, borderRadius: '14px', padding: '24px', boxShadow: '0 4px 14px rgba(0,0,0,0.06)' }}>
            ...
            <p style={{ color: theme.muted, fontSize: '14px', margin: 0 }}>
              Akash S — an independent project focused on student career safety.
            </p>
          </div>
        </div>
      )}
